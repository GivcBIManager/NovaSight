...
class TenantCreateSchema(Schema):
    ...
    slug = fields.Str(
        ...
        validate=[
            ...
            validate.Regexp(
                r'^[a-z][a-z0-9_-]*$',
                error="Slug must start with a letter and contain only lowercase letters, numbers, hyphens, and underscores"
            )
        ],
        metadata={"description": "Unique tenant identifier for URLs and database schemas"}
    )
    plan = fields.Str(
        load_default='basic',
    ...
    def validate_slug(self, value):
        """Validate slug doesn't use reserved words."""
        reserved_words = ['admin', 'api', 'system', 'public', 'internal', 'root']
...
class TenantUpdateSchema(Schema):
    ...
    settings = fields.Dict(
        metadata={"description": "Tenant-specific configuration (merged with existing)"}
    )
    status = fields.Str(
...
class TenantUsageSchema(Schema):
    ...
    storage_gb = fields.Float(metadata={"description": "ClickHouse storage usage in GB"})
    users_count = fields.Int(metadata={"description": "Number of active users"})
    datasources_count = fields.Int(metadata={"description": "Number of data connections"})
    dashboards_count = fields.Int(metadata={"description": "Number of dashboards"})
    queries_last_30d = fields.Int(metadata={"description": "Query count in last 30 days"})
    dag_configs_count = fields.Int(metadata={"description": "Number of DAG configurations"})
