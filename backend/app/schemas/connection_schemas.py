...
class ConnectionConfigSchema(BaseModel):
    ...
    def validate_host(cls, v):
        ...
        try:
            ...
            # Log warning for private IPs
            if ip.is_private or ip.is_loopback:
                import logging
                logging.getLogger(__name__).warning(f"Private/loopback IP detected: {v}")
        except ValueError:
...
class ConnectionCreateSchema(BaseModel):
    ...
    def validate_name(cls, v):
        ...
        if not re.match(r'^[a-zA-Z][a-zA-Z0-9_\- ]*$', v):
            raise ValueError('Name must start with a letter and contain only letters, numbers, spaces, hyphens, and underscores')
        return v
