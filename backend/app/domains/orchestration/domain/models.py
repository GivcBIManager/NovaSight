"""
NovaSight Orchestration Domain — Models
=========================================

SQLAlchemy models for DAG configuration, versioning, and task management.

Canonical location: ``app.domains.orchestration.domain.models``

Changes from legacy ``app.models.dag``:
- ``DagConfig`` adopts ``TenantMixin`` (removes manual ``tenant_id`` column)
- ``DagConfig`` adopts ``TimestampMixin`` (removes manual ``created_at`` / ``updated_at``)
- Removed explicit ``tenant`` relationship (now provided by ``TenantMixin``)
"""
...
from sqlalchemy import (
    String, Text, Integer, DateTime, Boolean, ForeignKey,
    Enum as SQLEnum,
)
...
# ---------------------------------------------------------------------------
# DagConfig — adopts TenantMixin + TimestampMixin
# ---------------------------------------------------------------------------

class DagConfig(TenantMixin, TimestampMixin, db.Model):
    """
    ...
    schedule_preset = db.Column(String(50), nullable=True)
    ...
    # Audit — created_by is NOT a mixin field; kept explicitly
    ...
    versions = relationship(
        "DagVersion",
        back_populates="dag_config",
        order_by="DagVersion.version.desc()",
    )
    tasks = relationship(
        "TaskConfig",
        back_populates="dag_config",
    ...
    def to_dict(
        ...
        if include_versions:
            result["versions"] = [v.to_dict() for v in self.versions[:10]]
...
# ---------------------------------------------------------------------------
# DagVersion — no mixin needed (no tenant FK, simple audit)
# ---------------------------------------------------------------------------
...
class DagVersion(db.Model):
    ...
    # Parent DAG config
    dag_config_id = db.Column(
        UUID(as_uuid=True),
        ForeignKey("dag_configs.id"),
        nullable=False,
        index=True,
    )
...
# ---------------------------------------------------------------------------
# TaskConfig — child of DagConfig, no direct tenant FK
# ---------------------------------------------------------------------------
...
class TaskConfig(db.Model):
    ...
    # Task type
    task_type = db.Column(SQLEnum(TaskType), nullable=False)
