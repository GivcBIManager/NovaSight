"""
NovaSight Identity Domain — Auth Schemas
=========================================

Canonical location: ``app.domains.identity.schemas.auth_schemas``
...
class RegisterRequest(BaseModel):
    ...
    email: EmailStr = Field(..., description="User email address")
    password: str = Field(
        ..., min_length=12, description="User password (min 12 chars)"
    )
    name: str = Field(
        ..., min_length=1, max_length=100, description="User display name"
    )
    tenant_slug: str = Field(
        ..., min_length=3, max_length=50, description="Tenant slug"
    )
    ...
    def validate_tenant_slug(cls, v: str) -> str:
        ...
        if not re.match(r"^[a-z][a-z0-9-]{2,49}$", v):
            raise ValueError(
                "Tenant slug must start with letter, contain only lowercase "
